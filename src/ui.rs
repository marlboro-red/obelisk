...
fn render_dashboard(f: &mut Frame, area: Rect, app: &App) {
    ...
    // When cols < 100: hide sparkline and give full width to event log
    ...
    if v_chunks.len() > 1 {
        if compact_cols {
            // < 100 cols: hide sparkline, full-width single-line event log
...
fn render_info_bar_compact(f: &mut Frame, area: Rect, app: &App) {
    ...
    let line = Line::from(vec![
        ...
        Span::styled(" │ ", Style::default().fg(MUTED)),
        Span::styled(format!("{}✓", app.total_completed), Style::default().fg(ACCENT)),
        Span::styled(" ", Style::default().fg(MUTED)),
        Span::styled(
            format!("{}✗", app.total_failed),
            Style::default().fg(if app.total_failed > 0 { DANGER } else { MUTED }),
...
/// Accounts for compact mode when terminal dimensions are small.
